...
output "firewall_log_destinations" {
  ...
  value = concat(
    ...
    var.enable_s3_archival ? [
      { log_type = "ALERT", log_destination_type = "S3", log_destination = { bucket_name = aws_s3_bucket.logs.id } },
      { log_type = "FLOW", log_destination_type = "S3", log_destination = { bucket_name = aws_s3_bucket.logs.id } },
    ] : [],
  )
}
